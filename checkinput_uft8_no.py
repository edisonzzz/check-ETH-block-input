...
def write_transactions_input_to_txt(block_number, transactions_input):
    if transactions_input:
        with open(f"block_{block_number}_transactions_input.txt", "w") as file:
        ...
        print(f"区块 {block_number} 中成功解码的交易 input 信息已写入到 block_{block_number}_transactions_input.txt 文件中。")
    else:
        print(f"获取区块 {block_number} 中的交易信息失败。")

# 获取区块号区间内的所有交易 input 信息
def get_block_transactions_input_range(start_block, end_block):
    for block_number in range(start_block, end_block + 1):
        block_transactions_input = get_block_transactions_input(block_number)
        if block_transactions_input:
            write_transactions_input_to_txt(block_number, block_transactions_input)

if __name__ == "__main__":
    # 指定要查询的区块号区间
    start_block = 9882730  # 区块号起始
    end_block = 9882740  # 区块号结束
    get_block_transactions_input_range(start_block, end_block)
